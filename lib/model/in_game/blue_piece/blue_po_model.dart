...
final class BluePoModel extends BluePieceBaseModel {
  BluePoModel({required super.x, required super.y})
    : super(
        team: Team.blue,
  ...
  void searchActionable(InGameBoardStatus statusBoard) {
    ...
    /// 위
    for (int i = y - 1; i >= 0; i--) {
      final breakNow = bluePoStatusProcessing(x, i);
      if (breakNow) break;
    }
    ...
    /// 왼쪽
    for (int i = x - 1; i >= 0; i--) {
      final breakNow = bluePoStatusProcessing(i, y);
      if (breakNow) break;
    }
    ...
    if (x == 3 && y == 7) {
      for (int i = x + 1, j = y + 1; i <= 5 && j <= 9; i++, j++) {
        final breakNow = bluePoStatusProcessing(i, j);
        if (breakNow) break;
      }
    } else if (x == 5 && y == 7) {
